...
async def process_audio(client, event, url, cookies_env_var=None):
    ...
    random_filename = f"@vibeshiftbots{event.sender_id}"
    ...
    try:
        ...
        if os.path.exists(download_path):
            def edit_metadata():
                ...
                audio_file.tags["TPE1"] = TPE1(encoding=3, text="@vibeshiftbots")
                audio_file.tags["COMM"] = COMM(encoding=3, lang="eng", desc="Comment", text="Processed by @vibeshiftbots")
...
async def fetch_video_info(url, ydl_opts, progress_message, check_duration_and_size):
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if check_duration_and_size:
            ...
            if estimated_size and estimated_size > 2 * 1024 * 1024 * 1024:   # 2 GB
...
async def process_video(client, event, url, cookies_env_var, check_duration_and_size=False):
    ...
    try:
        ...
        title = info_dict.get('title', 'Powered by @vibeshiftbots')
        ...
        if os.path.exists(download_path) and os.path.getsize(download_path) > SIZE:
            ...
            await prog.delete()
            return  # <-- Prevent falling through to the second upload block
